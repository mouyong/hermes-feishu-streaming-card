...
当前发布版本为 `4.6.6`。范围：审批回执确认后精简重复、运行工具与前一步可见、终态失败工具保留、原生通知 15 秒收尾及跨重启期限恢复。自动化通过不等于真实客户端验收通过。

全量测试、精确合并 CI、资产校验和公开 tag 普通安装仍是发布门禁。桌面/手机与生产结果单独登记在 [V4.6.6 验收清单](wiki/feishu-acceptance-v4.6.6.md)；未复验的现场问题继续保留原边界。
